...
<!--
Descriptive analysis of environmental monitoring data: CO2 concentrations,
temperature, humidity, and air exchange rates (AER) in the hospital waiting area.

Two Aranet4 CO2 monitors were deployed (devices 272D2 and 25247). AER is
estimated from CO2 decay using two methods:
  - Transient mass balance (TMB): fits a dynamic CO2 model
  - Steady-state model (SSM): assumes equilibrium CO2 concentration

Reads from:
  - data-clean/environmental/co2-temp-humidity.csv  (5-min interval sensor data)
  - data-clean/environmental/air-exchange-rate.csv   (daily AER estimates)
  - data-clean/mapping_dates_interventions.csv       (study phase timeline)

Outputs:
  - results/environmental_co2_over_time.rds                  (plot: diurnal CO2)
  - results/environmental_co2_over_time_by_device.png        (plot: sensor comparison)
  - results/environmental_ventilation_model_comparison.png   (plot: TMB vs SSM)

Sections:
  1. Load data — read sensor measurements and AER, merge with study phases
  2. Descriptive summaries — CO2, temperature, humidity, and AER statistics
  3. CO2 over time — diurnal CO2 concentration profiles
  4. Correlation between CO2 measurements — compare the two Aranet4 monitors
  5. Air exchange rate by model — compare TMB vs SSM ventilation estimates
-->
...
Load CO2, temperature, humidity, and air exchange rate (AER) data.
...
# Summary statistics for CO2, temperature, humidity, and AER by study phase
# co2
co2_summary <- co2_df %>%
...
# absolute ventilation rates
aer_df %>%
  filter(device == "Aranet4 272D2") %>%
  # Volume of the waiting area (m^3); used to convert AER to absolute ventilation rate (L/s)
  mutate(vent_rate = aer_tmb * 1178.70 / 3.6) %>%
  summarise(across(vent_rate, .fns = list(
    median = ~ median(.x),
    q25 = ~ quantile(.x, .25),
    q75 = ~ quantile(.x, .75)
  )))
...
# air exchange rate steady state model
aer_ssm_summary <- aer_df %>%
  filter(device == "Aranet4 272D2") %>%
  summarise(
    median_aer = median(aer_ssm, na.rm = TRUE),
    low_aer = quantile(aer_ssm, 0.25, na.rm = TRUE),
    upper_aer = quantile(aer_ssm, 0.75, na.rm = TRUE)
  ) %>%
  mutate_all(~ round(.x, 1))
aer_ssm_summary
...
co_out <- aer_df %>%
  filter(device == "Aranet4 272D2") %>%
  summarise(across(c(Cr_tmb, Cr_ssm), .fns = list(
    median = ~ median(.x),
    q25 = ~ quantile(.x, .25),
    q75 = ~ quantile(.x, .75)
  )))
co_out %>%
  gather()
cor.test(aer_df$Cr_tmb, aer_df$Cr_ssm)
```
...
```{r co2 over time}
# Diurnal CO2 concentration profiles
...
# Compare CO2 readings from the two Aranet4 monitors
...
```{r aer by model}
# Compare AER estimates from transient mass balance (TMB) vs steady-state (SSM) models
