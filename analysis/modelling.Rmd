...
<!--
Analysis of TB transmission modelling results from Monte Carlo simulations.

The model estimates Mtb infection risk using a spatiotemporal diffusion model
(non-uniform mixed, NUM) which accounts for proximity between infectious and
susceptible individuals, and compares it to a uniform well-mixed model (UM)
that assumes equal exposure everywhere.

Infection probability follows the Wells-Riley equation: P = 1 - exp(-Q),
where Q is the cumulative inhaled quanta dose.

Intervention effects are estimated using Bayesian regression (brm_multiple)
fitted across all Monte Carlo simulation draws to propagate parameter
uncertainty.

Reads from:
  - data-clean/tracking/person-features.csv                          (demographics)
  - modelling-results/uncertain_all_equal_infectious/                (main analysis)
  - modelling-results/uncertain_confirmed_more_infectious/           (scenario 1)
  - modelling-results/uncertain_hiv_more_infectious/                 (scenario 2)
  - modelling-results/uncertain_all_infectious/                      (scenario 3)
  - data-clean/mapping_dates_interventions.csv                        (study phases)

Outputs:
  - results/modelling_risk_per_day.png                                (daily infections)
  - results/modelling_risk_overall.png                                (NUM vs UM comparison)
  - results/modelling_adjusted_rate_ratio_per_day.png                 (rate ratios)
  - results/modelling_estimated_reduction_in_infections.rds           (intervention effects)
  - results/modelling_risk_by_infectiousness_assumptions.png          (sensitivity)
  - results/modelling_hourly_risk_by_group.png                        (stratified risk)

Sections:
  1. Risk of infection — load results, compute daily expected infections
  2. Total risk by model — compare NUM (with proximity) vs UM (without)
  3. Person-time adjusted rate ratio — isolate proximity contribution per day
  4. Estimated reduction in infections — Bayesian regression of intervention effects
  5. Estimated prevented infections — counterfactual prediction
  6. Sensitivity analyses — alternative infectiousness assumptions (3 scenarios)
  7. Stratification — infection rate by gender and healthcare worker status
-->
...
```{r risk over time}
# Load simulation results, compute per-person infection probability
# via Wells-Riley equation: P = 1 - exp(-dose)
# person features
...
# Daily expected Mtb infections with credible intervals
# plot risk over time
risk_time_pl <- ggplot(
  data = avg_daily_risk,
  ...
  scale_colour_manual(
    values = paletteer_d("PrettyCols::Lively"),
    name = "Study phase",
    labels = c(
      "Baseline",
...
```{r risk overall}
# Compare total infections: non-uniform mixed model (NUM) vs uniform mixed (UM)
# overall risk
...
# Person-time adjusted rate ratio (NUM/UM) per day
...
# Bayesian Gaussian regression of log-transformed infections on study phase,
# with offsets for well-mixed risk and duration. brm_multiple() fits across
# all Monte Carlo draws to propagate simulation uncertainty.
...
# Counterfactual: predict infections under baseline conditions for
# intervention days, compute prevented infections as the difference.
...
# Sensitivity: compare total infections under different infectiousness assumptions
# files
...
all_infectious_files <- list.files(
  "modelling-results/uncertain_all_infectious/",
  pattern = "\\.csv$", full.names = TRUE, recursive = TRUE
)
...
all_infectious_df <- map_dfr(
  all_infectious_files,
  read_number_of_infections
)
...
uncertain_df <- bind_rows(
  ...
  all_infectious_df %>%
    mutate(
      assumption =
        "**Scenario 3**<br>All presumptive infectious"
    )
) %>%
...
```{r}
# Stratify infection rate by gender and healthcare worker status
