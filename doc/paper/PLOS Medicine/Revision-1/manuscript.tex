...
We collected clinical, environmental and person-tracking data for 52 days from June 17 to August 22, 2024, in the waiting hall of a hospital in Lusaka, Zambia (Figure \ref{fig:1}A). \added{The waiting area comprised approximately 393\,m\textsuperscript{2} of floor space with a diagonal ceiling (height: 3\,m to 4.4\,m), yielding an estimated volume of approximately 1,179\,m\textsuperscript{3} for the waiting hall (when assuming a ceiling height of 3\,m for simplicity during subsequent modelling and analysis).}\label{chg:R2.11setting}

The hospital is located in a large settlement of formal and semi-formal housing where TB and HIV are highly prevalent. It provides tuberculosis, HIV, and general outpatient services. \added{It serves exclusively walk-in patients, including both emergency and elective cases. Without dedicated crowd management, attendees typically sat as close to the doctors' offices as possible, resulting in concentrated clustering in the front rows of the waiting area. As is common in the African setting, patients were also frequently accompanied by friends or relatives, further increasing crowding in the waiting area.}\label{chg:R2.12setting}\label{chg:R2.13relatives} 

\subsection{\textcolor{blue}{Study interventions}}\label{chg:R2.12}\label{study-interventions}

The hospital operates continuously, 24 hours a day, seven days a week. We collected data during the busy hours from 6 am to 6 pm under three different study conditions (see Table \ref{tab:1} for details): a baseline phase (24 days) with routine clinic conditions for nine days at the start and fifteen days at the end of the study; a first intervention phase (14 days) with a redesigned waiting area to achieve a more uniform spatial distribution of patients, as well as signage and reminders to encourage physical distancing; and a second intervention phase (14 days) that added a patient flow system \added{aimed at reducing the number of random close-contact encounters between attendees by directing movement through the waiting area}\label{chg:R2.13oneway}. \added{A graphical representation of the interventions is shown in Figure \ref{fig:1}B–C. The interventions targeted spatial clustering and close-proximity contact among attendees, not waiting time or time spent in the clinic; any effect on waiting or visit time would be incidental.}\label{chg:R2.32}

We \added{initially} aimed for a sample of 14 days per study phase to capture sufficient variation in crowding, ventilation and TB burden\added{, balancing data needs with the operational burden on local study staff. Two full weeks per phase covered all days of the week twice, enabling adjustment for day-of-week effects}\label{chg:R1.1}. \added{The pre-intervention baseline was shorter than planned because several days were excluded due to electricity or internet outages that interrupted tracking. To compensate, we added 15 post-intervention baseline days. This proved analytically beneficial: as patient volumes declined over the study period, the two baseline phases bookended the interventions with higher (start) and lower (end) attendance, allowing verification of temporal adjustments. All infrastructure changes were temporary and removed between phases, and study staff were instructed not to continue intervention activities during baseline periods, limiting carryover effects.}\label{chg:R1.2} 

Descriptive data are presented for all four phases (pre-intervention baseline, first and second intervention, post-intervention baseline) and the modelling approach employed for effect estimation was constructed to explicitly account for and remove observed seasonal trends, thereby mitigating potential bias.
...
Throughout the study, \replaced{surgical mask use was high among hospital staff but rare among other attendees. However, mask use was not modelled, in order to isolate the impact of the proximity-targeting interventions from other IPC measures in place}{mask use was high among hospital staff but rare among attendees}.\label{chg:R2.33}

\subsection{Person-movement tracking}\label{person-movement-tracking}

As in previous studies in a South African clinic \cite{banholzer2025PLOSComputBiol,zurcher2022JInfectDis}, we used an anonymous person tracking system with optical sensors (Xovis, Zollikofen, Switzerland) to track person movements at 1-second intervals in the main waiting area of the Zambian hospital. \added{The tracking system is commercially developed and widely deployed in airports, retail, and public transport settings. The sensor configuration for our study was optimised in collaboration with a company specialising in the installation of these tracking systems (ASE AG, Z\"urich, Switzerland).}\label{chg:R2.2} 
...
We examined spatial density in the waiting area by rasterising the space into a grid of \emph{n} squared cells each with a length of 0.5m. To approximate close-proximity interactions, the person counts \emph{p} were smoothed using a Gaussian kernel density estimator with a bandwidth of 1.5m. Spatial density was quantified with the Gini coefficient \(G = \frac{1}{2n^{2}\overline{p}\ }\sum_{i = 1}^{n}{\sum_{j = 1}^{n}{|p_{i} - p_{j}|}}\). This coefficient compares the observed person counts with a uniform distribution, with larger values indicating higher clustering (larger deviation from a uniform spread of people). \added{A coefficient of 0 would mean that the clinic attendees were perfectly uniformly distributed across the waiting area, whereas a value of 1 would mean that all attendees clustered together in a single cell. The Gini coefficient is a well-established measure of inequality and has been applied in spatial contexts \cite{garcia-montalvo2021,han2016IntJHealthGeographics,rey2013LettSpatialResourSci}. To assess sensitivity to these chosen kernel and measure, we also computed spatial clustering using an exponential kernel and using Shannon entropy as an alternative measure of the spatial distribution of clinic attendees (Supplementary Table \ref{tab:S3}).}\label{chg:R2.5}\label{chg:R3.3}
...
\added{Our model has important limitations related to environmental assumptions. We did not directly measure outdoor CO\textsubscript{2} concentrations, which may introduce bias into the estimated ventilation rates if outdoor levels fluctuated during the day. We also did not assess airflow patterns within the waiting area. While the space was naturally ventilated with predominantly outdoor replacement air, localised air currents---driven by wind, door openings, ceiling fans, or thermal gradients---could redistribute infectious aerosols in ways not captured by our model, which assumes radial diffusion within a horizontally well-mixed airspace. This may affect the spatial quanta concentration and thus the modelled proximity-dependent transmission risk. Computational fluid dynamics (CFD) models offer an alternative by simulating detailed airflow patterns and their influence on aerosol transport \cite{ding2025BuildEnviron}. However, CFD models are computationally expensive and primarily designed for experimental or static environments, making their application to real-world clinical settings with dynamic occupancy and continuously changing conditions difficult. In particular, our modelling approach required computational efficiency because many Monte Carlo simulations were needed to propagate uncertainty in generation, diffusion, and removal parameters. Emerging rapid assessment methods that combine behavioural data with simplified airflow assumptions may help bridge this gap in future studies \cite{ding2025BuildEnviron}. We acknowledge that incorporating airflow data---whether through direct measurement, CFD simulation, or hybrid approaches---would refine the spatial component of our transmission model and is an important direction for future research.}\label{chg:R2.27}
