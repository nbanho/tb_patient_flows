...
Clinical data have not been deposited, as the individual-level records pose an unnecessary risk of inadvertent patient disclosure (see also \ref{r:2.24}). However, all preprocessed outputs necessary to reproduce the modelling results, such as linked tracking tables indicating TB patient movements but without personal attributes such as age, sex, or HIV status, are provided in the repository. The repository contains a detailed README with step-by-step instructions for downloading the data and reproducing all results.}
...
\item We updated Supplementary Text B to explicitly compare their their respective assumptions about outdoor (replacement air) CO\textsubscript{2} concentrations.
