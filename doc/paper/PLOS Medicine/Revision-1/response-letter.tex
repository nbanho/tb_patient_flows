...
\response{We have revised and clarified the Data Availability Statement. Anonymized person-tracking and environmental data, along with all code, are publicly available through a linked OSF and GitHub repository (OSF: \url{https://doi.org/10.17605/OSF.IO/P3W24}; GitHub: \url{https://github.com/nbanho/tb_patient_flows}). Data are released under a CC BY-NC 4.0 license and code under an MIT license, with a data use agreement as required by the National Health Research Authority (NHRA) of Zambia. Clinical data have not been deposited, as the individual-level records pose an unnecessary risk of inadvertent patient disclosure (see also \ref{r:2.24}). However, all preprocessed outputs necessary to reproduce the modelling results---including linked tracking tables indicating TB patient movements, but without personal attributes such as age, sex, or HIV status---are included in the repository. The repository contains a detailed README with step-by-step instructions for downloading the data and reproducing all results.}
